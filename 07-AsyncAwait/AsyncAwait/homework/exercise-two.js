...
async function problemA() {
  ...
  const promesas = ["poem-two/stanza-01.txt", "poem-two/stanza-02.txt"].map(
    (txt) => promisifiedReadFile(txt)
  );

  const respuesta = await Promise.all(
    ["poem-two/stanza-01.txt", "poem-two/stanza-02.txt"].map((txt) =>
      promisifiedReadFile(txt)
    )
  );

  console.log("respuesta soy -> ", respuesta);

  for (const promise of respuesta) {
    blue(promise);
  }
...
async function problemB() {
  ...
  // AsyncAwait version
  const respuesta = await Promise.all(
    filenames.map((txt) => promisifiedReadFile(txt))
  );
  respuesta.forEach((stanza) => blue(stanza));
  console.log("done");
}
...
async function problemD(req,res) {
  /* * * * * * * * * * * * * * * * * * * * * * * * * * * *
   *
  ...
  try {
    for (const file of filenames) {
      const stanza = await controlador(file);
      blue(stanza);
    }
    console.log("done");
  } catch (error) {
